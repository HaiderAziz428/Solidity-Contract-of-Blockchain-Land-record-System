...
graph TB
    subgraph "Off-Chain Layer"
        A[Government Backend] -->|Verifies NADRA DB| B[Verification Oracle]
        C[IPFS Network] -->|Stores Documents| D[Land Deeds/Maps]
    end
    
    subgraph "On-Chain Layer"
        B -->|Calls storeVerifiedLandRecord| E[LandRegistry Contract]
        E -->|Mints| F[ERC-721 NFT]
        E -->|Stores Hash| G[Privacy Layer]
    end
    
    subgraph "User Layer"
        H[Citizens] -->|Register| E
        H -->|Transfer| F
        I[Heirs] -->|Approve/Dispute| E
    end
```
...
sequenceDiagram
    ...
    Backend->>Contract: initiateInheritance(oldLandId, [heir1, heir2], [newLandId1, newLandId2])
    Contract->>Contract: Set status = PENDING_INHERITANCE
    Heir1->>Contract: approveSuccessionPlan(oldLandId)
    Heir2->>Contract: approveSuccessionPlan(oldLandId)
    Contract->>Contract: _executeInheritance()
    Contract->>Contract: Burn old NFT
    Contract->>Heir1: Mint new NFT (newLandId1)
    Contract->>Heir2: Mint new NFT (newLandId2)
```
